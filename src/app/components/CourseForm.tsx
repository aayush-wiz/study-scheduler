...
const MAX_COURSE_HOURS = 168; // Maximum hours in a week (changed from 40)
...
export default function CourseForm({
  ...
  const validateHours = (value: string): boolean => {
    ...
    // Check if it would exceed available hours (if set)
    if (availableHours > 0 && numValue > availableHours) return false;
  ...
  const validateForm = () => {
    ...
    if (!validateHours(hoursNeeded)) {
      ...
      } else if (availableHours > 0 && numHours > availableHours) {
        setError(`Hours cannot exceed available hours (${availableHours})`);
      } else {
        setError(
    ...
    if (numHours > remainingHours && availableHours > 0) {
      setError(`Only ${remainingHours} hours available for allocation`);
      return false;
    }
